...
def create_submission(prediction, score):
    ...
    sub_file = 'submission_'+str(score)+'_' + \
        str(now.strftime("%Y-%m-%d-%H-%M"))+'.csv'
    #sub_file = 'prediction_training.csv'
    print('Creating submission: ', sub_file)
    pd.DataFrame({'Id': test['Id'].values, 'SalePrice': prediction}).to_csv(
...
class ensemble(object):
    ...
    def fit_predict(self, train, test, ytr):
        ...
        grid = GridSearchCV(estimator=self.stacker,
                            param_grid=param_grid, n_jobs=1, cv=5, scoring=RMSE)
        grid.fit(S_train, y)
        try:
            print('Param grid:')
            print(param_grid)
